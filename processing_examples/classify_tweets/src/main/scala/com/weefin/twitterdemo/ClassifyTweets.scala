...
object ClassifyTweets extends App with LazyLogging {
	...
	env.addSource(consumer).flatMap(FlatMapDefinedOption[Tweet]).map(SimpleStatus(_)).map(MapClassifyByHashtags)
		.map(new ClassifiedSimpleStatus(_)).addSink(producer)
	env.execute(jobName)
	
	private case class ClassifiedSimpleStatus(status: SimpleStatus, mainClass: String) {
		def this(classifiedEntity: ClassifiedEntity[SimpleStatus]) = this(classifiedEntity.entity,
