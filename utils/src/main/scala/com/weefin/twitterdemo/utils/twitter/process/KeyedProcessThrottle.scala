...
class KeyedProcessThrottle[K, I, O](throttle: Time, f: I => O) extends KeyedProcessFunction[K, I, O] {
	...
	override def processElement(value: I,
		...
		out.collect(f(value))
		state.update(true)
		ctx.timerService.registerProcessingTimeTimer(ctx.timerService.currentProcessingTime() + throttle
			.toMilliseconds)
	}
}

object KeyedProcessThrottle {
	def apply[K, I, O](throttle: Time, f: I => O) = new KeyedProcessThrottle[K, I, O](throttle, f)
}